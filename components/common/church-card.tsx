...
const ChurchCard = ({ church, onClick }: ChurchCardProps) => {
        ...
        <div className="md:hidden">
          <div className="flex gap-3">
            ...
            <div className="flex-shrink-0">
              {church.image ? (
                <div className="w-16 h-16 rounded-xl overflow-hidden bg-[#f5f1eb]">
                  <Image
                    src={church.image}
                    alt={church.name}
                    width={64}
                ...
                <div className="w-16 h-16 rounded-xl bg-gradient-to-br from-[#5d4a2f] to-[#8b6f47] flex items-center justify-center">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between mb-1">
                <div className="flex-1 min-w-0">
                  <h3 className="text-base font-bold text-[#3d2817] mb-1 truncate">{church.name}</h3>
                  {church.denomination && (
                ...
                {church.distance && (
                  <span className="text-xs text-[#6b5d4a] font-medium flex-shrink-0 ml-2">
                    {church.distance.toFixed(1)} mi
                  </span>
                )}
              </div>
              <div className="flex items-center gap-2 text-xs text-[#6b5d4a]">
                <MapPin size={12} className="flex-shrink-0 text-[#8b6f47]" />
                <span className="truncate">{church.city}, {church.state}</span>
              </div>
              {church.phone && (
                <div className="flex items-center gap-1.5 text-xs text-[#6b5d4a] mt-1">
        ...
        <div className="hidden md:block space-y-4">
          <div className="flex gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-start justify-between mb-2">
                <div className="flex-1 min-w-0">
                  <h3 className="text-xl font-bold text-[#3d2817] mb-1 truncate">{church.name}</h3>
                  {church.denomination && (
              ...
              {church.description && (
                <p className="text-sm text-[#6b5d4a] line-clamp-2">{church.description}</p>
              )}
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            {church.serviceTimes && church.serviceTimes.length > 0 && (
              <div className="flex items-start gap-2 text-sm text-[#6b5d4a]">
                <Calendar size={16} className="mt-0.5 flex-shrink-0 text-[#8b6f47]" />
  ...
  return (
    <Link href={`/app/church/${church.id}`}>
      {cardContent}
    </Link>
  );
};
